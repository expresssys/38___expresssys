...
public class HandoverController {
	...
	@ResponseBody
	public List<Car> selectcar(String ctype,HttpServletRequest request){
		Car car=new Car();
		car.setCtype(ctype);
		List<Car> list = null;
		try {
			list = carBiz.findAllCar(car);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return list;
	...
	public @ResponseBody JsonModel findBy(Handover h,Orders r,HttpServletRequest request){
		...
		if(orders.get(0).getOstatus()==100){
			jm.setMsg(consts.get("1"));
		}else{
	...
	@ResponseBody
	public Map<String,Object> updateHandover(Handover handover,Route route){
		Map<String,Object> map=new HashMap<String,Object>();
		Orders orders = new Orders();
		orders.setOsid(handover.getOsid());
		...
		Map<String, Object> routes = this.routeBiz.findBy(param);
		
		int length = 0;
		if(routes!=null && routes.size()>0){
			List<Route> routess= (List<Route>)routes.get("rows");
			route = routess.get(0);
			String str = route.getRvia();
			String[] s = str.split("-");
			length  = s.length;
		}
		
		List<Handover> handovers = this.handoverBiz.findBy(handover);
		
		if(result>0){
			map.put("code", 1);
			if(handover.getHstatus()==2 && handovers.size()==(length-1)){
				orders.setOstatus(4);
				ordersBiz.updateStatus(orders);
			}else if(handover.getHstatus()==1 && handovers.size()==1){
				orders.setOstatus(3);
				ordersBiz.updateStatus(orders);
			}else if(handover.getHstatus()==2 && handovers.size()==1){
				orders.setOstatus(3);
				ordersBiz.updateStatus(orders);
			}
			System.out.println(length + "  " + handover.getHstatus());
		}else{
			map.put("code", 2);
		}
		return map;
